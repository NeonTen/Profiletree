 ...
 * @return array
 */
function profiletree_body_classes( $classes ) {
...
add_filter( 'body_class', 'profiletree_body_classes' );

/**
 * Add a pingback url auto-discovery header for single posts, pages, or attachments.
 */
function profiletree_pingback_header() {
	if ( is_singular() && pings_open() ) {
		printf( '<link rel="pingback" href="%s">', esc_url( get_bloginfo( 'pingback_url' ) ) );
	}
}
add_action( 'wp_head', 'profiletree_pingback_header' );
...
function highlight( $atts, $content = null ) { // phpcs:ignore
	return '<span class="text-primary-color">' . esc_html( $content ) . '</span>';
}
