...
class ProfileTree_Menu_Walker {
	...
	public function build_menu( $loc = '' ) {
		...
		$menu_html = '<ul class="parent | text-text-color dark:text-white font-semibold flex items-center gap-4 sm:gap-10">';
		...
		foreach ( $this->data as $link ) {
			...
			if ( $current && 'mobile' !== $loc ) {
				$classes = 'flex items-center text-primary-color gap-2';
			}
			if ( ! $current ) {
				$classes = 'flex items-center hover:text-primary-color gap-2 group-hover:text-primary-color';
			}
			if ( 'mobile' === $loc ) {
			...
			if ( ! empty( $link['children'] ) && is_array( $link['children'] ) && 'mobile' !== $loc ) {
				$caret = '<span class="fill-text-color group-hover:fill-primary-color group-hover:rotate-45 transition-all">' . get_svg( 'icons/menu-arrow', false ) . '</span>';
			}
			if ( ! empty( $link['children'] ) && is_array( $link['children'] ) && 'mobile' === $loc ) {
			...
			foreach ( $link['children'] as $child ) {
				...
				if ( empty( $child['children'] ) ) {
					$menu_html .= sprintf(
						'<a href="%s" %s class="hover:text-primary-color">%s</a>',
						$child['url'],
						$target,
						$child['title']
					);
